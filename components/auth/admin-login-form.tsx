    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (successData.success) {
                // STRICT CHECK: Ensure user is an admin
                if (successData.user?.role !== 'super_admin') {
                    // If not an admin, logout immediately (optional but recommended) or just show error
                    // Here we just show error and user remains technically "logged in" as a user but stuck on this page
                    // Better approach: Call logout API to clear the session since they shouldn't be here
                    try {
                ...
                // Use window.location for full page load - ensures auth state is fresh
